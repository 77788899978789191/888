/**
 * Project: Gungnir-Absolute — 数据折磨（DataTorment）
 *
 * 【子系统 39：常量即时擦除】
 *  - 运行时解出的常量在使用作用域结束后立即置 nil 并 collectgarbage，
 *    擦除时机随机（函数返回前 / 下一循环开始前）。运行时桩注入。
 *
 * 【子系统 42：数据过程化】
 *  - 用函数调用生成静态表，而非字面量。生成函数包含复杂逻辑，
 *    每次返回相同布局的表（由构建种子决定）。
 *
 * 【子系统 43：表键名混淆】
 *  - 键名经哈希变换，访问时通过元表 __index 动态解密（解密密钥
 *    由构建种子派生）。静态无法枚举真实键名。
 *
 * 【子系统 46：弱表与终结器隐式数据流】
 *  - 通过 __gc 元方法（newproxy 技巧）与弱表传递数据。
 *    静态分析无法追踪 GC 驱动的数据流。
 *
 * 【子系统 47：语义等价替换】
 *  - string.gsub → 手动循环 + byte 操作；string.rep → 手动拼接；
 *    string.upper → 手动 byte 偏移。消除标准库调用特征。
 */
import {
  ObfuscationPlugin, ObfuscationContext, Chunk, LuaNode,
} from '../core/types';
import {
  walk, createRawStatement, generateLuaIdentifier,
} from '../utils/helpers';

export class DataTormentPlugin implements ObfuscationPlugin {
  name = 'DataTorment';
   ...
   */
  private replaceStdlibCalls(ctx: ObfuscationContext, intensity: number): void {
    ...
    walk(ctx.ast, (node, parent) => {
      ...
      if (base.indexer !== '.') return;
      const baseBase = base.base as Record<string, unknown> | undefined;
      const id = base.identifier as { name?: unknown } | undefined;
      if (!baseBase || baseBase.type !== 'Identifier' || baseBase.name !== 'string') return;
      const fname = String(id?.name ?? '');

      if (!enabled.has(fname)) return;
      if (ctx.rng.next() > rate) return;

      const args = (n.arguments as LuaNode[]) ?? [];
  ...
  private isSimpleString(node: LuaNode): boolean {
    const n = node as unknown as Record<string, unknown>;
    return n.type === 'StringLiteral' || n.type === 'Identifier';
  }

  private isSimpleNumber(node: LuaNode): boolean {
    const n = node as unknown as Record<string, unknown>;
    return n.type === 'NumericLiteral' || n.type === 'Identifier';
  }

  /** 无 pattern 元字符的纯字面 pattern（可安全手动替换） */
  private isPlainPattern(node: LuaNode): boolean {
    const n = node as unknown as Record<string, unknown>;
  ...
  private replaceRep(ctx: ObfuscationContext, node: Record<string, unknown>, args: LuaNode[]): void {
    const f = generateLuaIdentifier(ctx.rng, '_rp', 6);
    // 注入手动 rep 实现为内联 IIFE
  ...
  private replaceUpperLower(ctx: ObfuscationContext, node: Record<string, unknown>, args: LuaNode[], delta: number): void {
    const f = generateLuaIdentifier(ctx.rng, '_cl', 6);
    const sArg = this.exprToLua(args[0]);
    // delta 混入两个常量的差（隐蔽化）
  ...
  private replaceGsubPlain(ctx: ObfuscationContext, node: Record<string, unknown>, args: LuaNode[]): void {
    const f = generateLuaIdentifier(ctx.rng, '_gs', 6);
    const sArg = this.exprToLua(args[0]);
  ...
  private replaceWithRaw(node: Record<string, unknown>, code: string): void {
  ...
  private exprToLua(node: LuaNode): string {
    const n = node as unknown as Record<string, unknown>;
    if (n.type === 'StringLiteral') return this.quote(String(n.value ?? ''));
   ...
   * 表内容由函数过程生成，静态不可直接读取。
   */
  private proceduralizeTables(ctx: ObfuscationContext, intensity: number): void {
    ...
    walk(ctx.ast, (node, parent) => {
      ...
      const fields = (n.fields as { type?: string }[] | undefined) ?? [];
      if (fields.length < minFields || fields.length > maxFields) return;
      for (const f of fields) {
        if (f.type === 'TableKey') return; // 键控表由 StringSplitting 管理
      }
      // 必须全是简单值（数字/字符串/布尔）——其余类型过程化会破坏语义
      for (const f of fields) {
        const v = (f as { value?: Record<string, unknown> }).value;
        if (!v) return;
        const vt = String(v.type ?? '');
        if (vt !== 'NumericLiteral' && vt !== 'StringLiteral' && vt !== 'BooleanLiteral') return;
      }
      ...
      this.proceduralize(ctx, n, fields as { value: Record<string, unknown> }[]);
      ctx.stats.constantsObfuscated++;
    });
  }

  /** {v1, v2, v3} → (function() local t = {} t[1]=v1 ... return t end)() */
  private proceduralize(ctx: ObfuscationContext, node: Record<string, unknown>, fields: { value: Record<string, unknown> }[]): void {
    const lines: string[] = [];
   ...
   */
  private injectRuntimeStub(ctx: ObfuscationContext): void {
    const f = generateLuaIdentifier(ctx.rng, '_dt', 8);
    const kf = `${f}k`;
    const pool = `${f}pool`;
    const weak = `${f}weak`;
    const wrap = `${f}wrap`;
