/**
 * Project: Gungnir-Absolute — 平台专属护盾（PlatformDelta）
 *
 * 【子系统 84：触摸注入友好】
 *  - 启动无阻塞等待：初始化负载经 task.defer/spawn 分散，
 *    不触发 Delta 的超时保护，响应触摸事件流畅。
 *
 * 【子系统 85：跨平台差异化混淆】
 *  - 运行时探测 system 平台标识（Android/iOS/桌面），不同平台
 *    走不同混淆分支（Android 轻量防御 / iOS 重型防御）——
 *    使跨平台对比分析更困难。
 *
 * 【子系统 89：任务调度器帧序扰乱】
 *  - 混用 task.wait / spawn / delay / defer 调度初始化步骤，
 *    执行顺序不固定（由构建种子派生权重），扰乱基于时序的
 *    动态分析。
 */
import {
...
import {
  generateLuaIdentifier, createRawStatement,
} from '../utils/helpers';

export class PlatformDeltaPlugin implements ObfuscationPlugin {
  name = 'PlatformDelta';
   ...
   */
  private injectPlatformShield(ctx: ObfuscationContext): void {
    const f = generateLuaIdentifier(ctx.rng, '_pd', 8);
    const sched = `${f}q`;
    const plat = `${f}p`;
    const exec = `${f}e`;
