...
/** 派生流：从主种子按需派生确定性随机数（独立于 RngState 的构建级熵流） */
...
export class SeedEngine {
   ...
   */
  derive(domain: string): DerivedStream {
    let counter = 0;
   ...
   */
  fragmentParams(): { mult: number; mod: number; expected: number } {
   ...
   */
  fingerprintParams(): { expected: number; mask: number } {
    // 期望值 = 指纹 8 片段异或折叠 + 与种子根混合
    let acc = 0;
    for (const f of this.fingerprint) acc = (acc ^ f) >>> 0;
