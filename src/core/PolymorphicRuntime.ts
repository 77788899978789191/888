/**
 * Project: Gungnir-Absolute — 多态运行时发射器（PolymorphicRuntime）
 *
 * 生成随每次构建完全不同的 GX-VM Lua 运行时（Lua 5.1 严格兼容，
 * 所有平台 API 均经 pcall 防护，可在 Delta/Gloop 与标准 Lua 5.1
 * 环境中无差别运行）。
 *
 * 覆盖子系统：
 *  1  随机构建种子引擎（16 闭包片段重组校验）
 *  2  全动态操作码映射 + 运行时轮换（仿射双射）
 *  4  双重解释器（runA = if 链 / runB = 链表树 + 分发表）
 *  5  指令集布局随机化（长度/操作码位置/操作数位置随机）
 *  6  虚拟机数据结构随机化（链表树 / 数组 / 散列表混合）
 *  7  解释器自变异（阈值 2000-5000：算法形态切换/分发表重排/重命名）
 *  8  运行时指令置换（树节点段等价重写）
 *  9  常量池多态加密（GX-Cipher 程序解密 + 随机缓存槽）
 * 10  反内存 Dump（缓存洗牌 / 程序重编码 / 指针链散布）
 * 11  构建指纹 8 处嵌入防嫁接
 * 12  异常处理逻辑虚拟化（VM 级 pcall + 守卫程序字节码化）
 * 13  汇编级 MBA（种子常量/阈值以算术恒等式嵌入）
 * 26  协程风暴（200-300 协程，硬上限 300 优雅降级）
 * 34/35/38/87 字符串全量加密 / 常量池 / S 盒 / 分页惰性加载
 * 40  环境因子（tick/PlaceId 可用则混入守卫）
 * 60  环境表白名单沙盒
 * 66/71/72/73/77 反沙箱 / 反调试 / 时序侧信道 / 环境篡改 / 反钩子
 * 74  时间炸弹（TTL 可配，默认 0 关闭）
 * 75/78 调用栈污染 / debug.getinfo 污染
 * 76/79 运行时内存自校验 / 自变异代码块
 * 81  反篡改触发链（完整性哈希 → 缓存布局数据流耦合）
 * 83/86 Dark Dex 实例树诱饵 / 反收录随机指纹
 * 85  跨平台差异化分支（Roblox / 通用环境）
 * 88  Remote 三层加密助手
 * 89  任务调度器帧序扰乱（task.defer/delay/spawn 混用）
 */
import { SeedEngine } from './SeedEngine';
import {
  VMLayout, OP, compileCipherProgram, ghash,
  bytesToLuaString, VMInstruction, serializeProgram,
} from './VMCodec';

export interface PoolEntry {
  id: number;
  /** 明文字节（字符串原文或数字的规范字符串形式） */
  plaintext: number[];
  /** 数字池条目（KN 取值时 tonumber） */
  isNumber: boolean;
  original: string;
}
...
export interface RuntimeOptions {
  /** 混淆强度 1-5 */
  intensity: number;
  /** 时间炸弹 TTL（秒），0 = 关闭【子系统 74】 */
  timeBombTtl: number;
  /** 反调试响应模式【子系统 71】 */
  antiDebugMode: 'silent' | 'corrupt';
  /** 协程风暴数量上限（硬上限 300）【子系统 26】 */
...
export interface RuntimeEmission {
  ...
  /** 开发调试：完整载荷字节（仅诊断用，生产流程不读取） */
  payloadDebug?: number[];
}

/** 构建期派生的运行时名称表（每次构建不同【子系统 51/86/92】） */
interface Names {
  [key: string]: string;
}

function mka(digitStream: () => number, used: Set<string>): string {
 ...
 */
function mbaConst(v: number, ds: () => number): string {
...
export function emitRuntime(
  ...
  const digitStream = (): number => ds.nextU32() / 4294967296;
  ...
  for (const k of nameKeys) N[k] = mka(digitStream, used);

  // ============ 编译常量池程序【子系统 9/34】 ============
  const junkDs = seedEngine.derive('junk-bytes');
  const junkStream = (): number => junkDs.nextU32() & 0xFF;
  ...
  for (const entry of pool) {
    const r = compileCipherProgram(layout, entry.plaintext, entry.id, junkStream);
  ...
  const pages: number[][] = [];
  const pageMap: { p: number; s: number; e: number }[] = [];
  let curPage: number[] = [];
  let curPageIdx = 1;
  ...
  const guardInsns: VMInstruction[] = [
    { op: OP.TICK, a: 0, b: 0 },
    { op: OP.JZ, a: 0, b: 3 },
    { op: OP.SYNC, a: 1, b: 0 },
    { op: OP.TRAP, a: 0, b: 0 },
  ...
  const guardProg = serializeProgram(layout, guardInsns, 0, junkStream);

  // ============ 完整性哈希（100 分片链【子系统 70】）============
  // PAYLOAD = 全部程序字节 + 守卫程序 + 全部分页密文
  const payloadBytes: number[] = [];
  for (const c of compiled) payloadBytes.push(...c.progString);
  payloadBytes.push(...guardProg.bytes);
  for (const p of pages) payloadBytes.push(...p);
  ...
  for (let st = 0; st < payloadBytes.length; st += sliceLen) {
    ...
    ih = ghash(payloadBytes.slice(st, end), ih);
  }
  ...
  // 乘法双射保证不同 id 永不共享槽位。
  const nextPrimeAbove = (n: number): number => {
    let x = Math.max(3, n);
    if (x % 2 === 0) x++;
    for (;; x += 2) {
      let isPrime = true;
  ...
  const mdsF = (): number => mds.nextU32() / 4294967296;

  // ============ 程序与分页的 Lua 数据字面量 ============
  const progDefs = compiled.map(c =>
    `${N.PROGS}[${c.entry.id + 1}] = ${bytesToLuaString(c.progString)}`
  ).join('\n');
  const pageDefs = pages.map((p, idx) =>
    `${N.PAGES}[${idx + 1}] = ${bytesToLuaString(p)}`
  ).join('\n');
  ...
  let fpFold = 0;
  for (const w of fpWords) fpFold = (fpFold * 31 + (w % (2 ** 31 - 1))) % (2 ** 31 - 1);
...
local ${N.FR} = {}
${seedEngine.fragments.map((v, i) =>
  `${N.FR}[${i + 1}] = function(...) return ${mbaConst(v % (2 ** 31 - 1), mdsF)} end`
).join('\n')}
...
-- 【铁律】变异互换的实现必须逐点等值，否则变异触发后解密全错：
--   FADD2(x,y)  = (2x + 2y - (x+y)) % 256 = (x+y) % 256 ≡ FADD  ✓
--   FSUB2(x,y)  = (x + 256 - y) % 256      = (x-y) % 256  ≡ FSUB  ✓
--   （旧 FADD2 为 (3x+3y-(x+2y))%256 = (2x+y)%256 ≠ x+y —— 曾致
--    间歇性解密错值：数字条目解出乱码 → tonumber → nil）
local ${N.FADD} = function(x, y) return (x + y) % 256 end
local ${N.FSUB} = function(x, y) return (x - y + 512) % 256 end
local ${N.FADD2} = function(x, y) return (x * 2 + y * 2 - (x + y)) % 256 end
...
local ${N.GUARD} = ${bytesToLuaString(guardProg.bytes)}
...
${pageDefs}
${progDefs}
${N.PROGS}[0] = ${bytesToLuaString(guardProg.bytes)}
${pageMapDefs}
${N.PAYL} = nil
do
  local parts = {}
