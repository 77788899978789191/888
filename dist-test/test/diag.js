...
async function main() {
    ...
    for (let iter = 0; iter < LOOPS; iter++) {
        ...
        const vmPlugin = orch.getPlugins().find(p => p.name === 'VMEngine');
        ...
        const gchkName = paylName
            ...
            : undefined;
        if (!paylName || !gchkName)
            continue;
        let prologue = em.prologue;
        // 失败时导出 PAYL 字节
        prologue = prologue.replace(`${paylName} = table.concat(parts)`, `${paylName} = table.concat(parts)
do
  local t = {}
